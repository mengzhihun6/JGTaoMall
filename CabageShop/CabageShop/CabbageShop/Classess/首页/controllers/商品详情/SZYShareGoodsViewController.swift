...
//        导航上面的按钮
        ...
        segmentedControl.tintColor = UIColor.red //颜色设定
...
//        第一页scrollView
        ...
        fuzhi.backgroundColor = kSetRGBColor(r: 250, g: 100, b: 10)
        ...
        // 提示信息
        height = height + 45 + 20
        let labHeight = KGetLabHeight(labelStr: "白菜淘仅提供技术支持, 请自行确保分享内容真实合法(如不使用淘宝内部优惠券, 好评返现等虚假描述)", font: UIFont.systemFont(ofSize: 15), width: kSCREEN_WIDTH - 20)
        let tishi = UILabel.init(frame: CGRect.init(x: 10, y: height, width: kSCREEN_WIDTH - 20, height: labHeight + 50))
        tishi.text = "白菜淘仅提供技术支持, 请自行确保分享内容真实合法(如不使用淘宝内部优惠券, 好评返现等虚假描述)"
    ...
    func secondClick() {
        ...
        for index in 0..<2 {
            ...
            if index == 0 {
                ...
                bun.setTitleColor(UIColor.black, for: .normal)
                bun.backgroundColor = kSetRGBColor(r: 251, g: 242, b: 222)
            } else {
                bun.setTitle("立即分享", for: .normal)
                bun.setTitleColor(UIColor.white, for: .normal)
                bun.backgroundColor = kSetRGBColor(r: 250, g: 66, b: 7)
            }
            bun.tag = 200 + index
