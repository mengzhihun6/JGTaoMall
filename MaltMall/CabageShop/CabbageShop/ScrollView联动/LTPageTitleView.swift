...
extension LTPageTitleView {
    ...
    private func setupButtonsLayout() {
        ...
        let subH = bounds.height - layout.bottomLineHeight - layout.subToTop
        for index in 0..<titles.count {
            let subW = glt_textWidths[index]
            let buttonReact = CGRect(x: upX, y: layout.subToTop, width: subW, height: subH)
            ...
            let bgColor = (index == 0 ? layout.selectedBgColor : layout.normalBgColor)
...
extension LTPageTitleView: LTPageViewDelegate {
    ...
    private func setupButtonStatusAnimation(upButton: UIButton, currentButton: UIButton)  {
        ...
        upButton.backgroundColor = layout.normalBgColor
        currentButton.backgroundColor = layout.selectedBgColor
    }
    ...
    private func setupIsClickScrollAnimation(index: Int) {
        ...
        for button in glt_buttons {
            if button.tag == index {
                ...
                button.backgroundColor = self.layout.selectedBgColor
            }else {
                if layout.isNeedScale {
                    button.transform = CGAffineTransform(scaleX: 1.0 , y: 1.0)
                }
                button.setTitleColor(self.layout.titleColor, for: .normal)
                button.backgroundColor = self.layout.normalBgColor
            }
        }
...
extension LTPageTitleView {
    ...
    private func setupLineViewX(offsetX: CGFloat) -> Bool {
        ...
        if layout.isColorAnimation {
            ...
            currentButton.backgroundColor = layout.normalBgColor
            nextButton.backgroundColor = layout.selectedBgColor
        }
        if layout.isNeedScale {
