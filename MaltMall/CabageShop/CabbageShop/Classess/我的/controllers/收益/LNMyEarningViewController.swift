...
//    fileprivate var showView = ArrowheadMenu()
    ...
    override func viewDidLoad() {
        ...
        head_bgImageView.backgroundColor = UIColor.black
        yellowView = UIImageView.init()
        yellowView.frame = CGRect(x: 10, y: 13 + navHeight, width: kSCREEN_WIDTH - 20, height: 148)
        self.view.addSubview(yellowView)
        yellowView.backgroundColor = UIColor.init(r: 243, g: 214, b: 181)
        yellowView.layer.cornerRadius = 10
        yellowView.clipsToBounds = true
        yellowView.isUserInteractionEnabled  = true
        
        let lable  = UILabel()
        lable.frame = CGRect(x: 0, y: 25, width: yellowView.width, height: 20)
        lable.text = "您通过麦芽淘已经赚了"
        lable.font = UIFont.systemFont(ofSize: 14)
        lable.textAlignment = NSTextAlignment.center
        yellowView.addSubview(lable)
        
        moneyLabel = UILabel.init()
        moneyLabel.frame = CGRect(x: 0, y: 55, width: yellowView.width, height: 33)
        moneyLabel.textAlignment = NSTextAlignment.center;
        moneyLabel.text = "¥0.00元"
        moneyLabel.font = UIFont.systemFont(ofSize: 18)
        yellowView .addSubview(moneyLabel)
        topView = LNTopSelectView.init(frame: CGRect(x: 0, y: 108, width: kSCREEN_WIDTH - 20, height: 44))
        topView.backgroundColor = UIColor.clear
        topView.normalColor = UIColor.black
...
//        request.setParam("" as NSObject, forKey: "")
        request.callGET(withUrl: LNUrls().kShouyi) { (response) in
            DispatchQueue.main.async {
                ...
                let string : String = OCTools().getStrWithFloatStr2(weakSelf?.EarningsModel.order.money)
                
                
                weakSelf?.moneyLabel.text = "¥\(string) 元"
...
//        }
        ...
        mainTableView = getTableView(frame: CGRect(x: 0, y: CGFloat(navHeight)+167, width: kSCREEN_WIDTH, height: kSCREEN_HEIGHT - CGFloat(navHeight) - 49), style: .grouped, vc: self)
