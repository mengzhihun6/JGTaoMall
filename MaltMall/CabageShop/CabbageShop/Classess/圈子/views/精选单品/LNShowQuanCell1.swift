...
//    @IBOutlet weak var comments_height: NSLayoutConstraint!
    ...
    @IBOutlet weak var yujizhuanLbl: UILabel!
...
//        yujizhuanBun.layoutButton(with: .left, imageTitleSpace: 9)
        ...
        share_button.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.left
        share_button.layoutButton(with: .left, imageTitleSpace: 5)
        
        buy_button.contentHorizontalAlignment = UIControl.ContentHorizontalAlignment.left
        buy_button.layoutButton(with: .left, imageTitleSpace: 5)
    ...
    @IBAction func CopyAction111(_ sender: Any) {
    ...
    func setupValues(images:[String],comments:[String]) {
        ...
        let count = images.count
         ...
         let lines = 4
        let kSpace = CGFloat(5)
        let kWidth = (kDeviceWidth - 20 - kSpace*3) / CGFloat(lines)
        let kHeight = kWidth
...
//            }
            if Defaults[kCommissionRate] == "0" || Defaults[kCommissionRate] == nil {
                yujizhuanLbl.isHidden = true
            } else {
                yujizhuanLbl.isHidden = false
                ...
                yujizhuanLbl.text = "预计赚：￥" + String.init(format:"%.2f",jieguo)
//                yujizhuanBun.setTitle("预计赚：￥" + String.init(format:"%.2f",jieguo), for: .normal)
            ...
            send_time.text = "建议发送时间" +  model.show_at
        }
    }
...
//        }
    ...
    @IBAction func copy111(_ sender: Any) {
        let pasteboard = UIPasteboard.general
        pasteboard.string = quan_content.text
