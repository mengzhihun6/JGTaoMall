//
//  SZYGoodNightViewController.swift
//  CabbageShop
//
...
class SZYGoodNightViewController: LNBaseViewController, LTTableViewProtocal {
    ...
    override func configSubViews() {
        mainTableView = getTableView(frame: CGRect(x: 0, y: 50, width: kSCREEN_WIDTH, height: view.height - 64 - 50 - 49), style: .grouped, vc: self)
        mainTableView.register(UINib.init(nibName: "LNShowQuanCell2", bundle: nil), forCellReuseIdentifier: identyfierTable)
...
extension SZYGoodNightViewController: UITableViewDelegate, UITableViewDataSource {
    
    func numberOfSections(in tableView: UITableView) -> Int {
        return goodResource.count//1
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return 1//goodResource.count
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: identyfierTable, for: indexPath) as! LNShowQuanCell2
        cell.selectionStyle = .none
        cell.model = goodResource[indexPath.section]
        return cell
    }
