...
class SZYScratchableIatexTableViewCell: UITableViewCell {
    ...
    override func awakeFromNib() {
        ...
        let W = kDeviceWidth / 5.0
        let H = kWidthScale(75)
        
        bg_view_height.constant = CGFloat(H * 2 + 110.0)
...
//        mainCollectionView = UICollectionView.init(frame: CGRect(x: 0, y: -kStatusBarH, width: kSCREEN_WIDTH, height: kSCREEN_HEIGHT+kStatusBarH-49), collectionViewLayout: layout)
        ...
        mainCollectionView?.snp.makeConstraints({ (ls) in
            ...
            ls.height.equalTo(H*2)
        })
        
        
        let AdIcon = UIImageView()
        AdIcon.image = UIImage(named: "home_AdIcon")
        bg_view.addSubview(AdIcon)
        AdIcon.snp.makeConstraints { (make) in
            make.top.equalTo(mainCollectionView!.snp_bottom).offset(5)
