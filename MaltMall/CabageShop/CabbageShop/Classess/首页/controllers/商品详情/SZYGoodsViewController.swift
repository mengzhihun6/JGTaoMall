...
//    fileprivate var topView = LNTopSelectView() //    顶部选择
    ...
    override func viewDidLoad() {
        ...
        shoucangBun.setImage(UIImage.init(named: "jg_hover_ss"), for: .normal)
        shoucangBun.setImage(UIImage.init(named: "jg_hover"), for: .selected)
...
//        接受APPdelegate里发送的通知
    ...
    override func configSubViews() {
        ...
        mainTableView = getTableView(frame: CGRect.zero, style: .grouped, vc: self)
//        优惠券详情显示
...
//                    kDeBugPrint(item: GoodsInformationModel.commission_rate)
                    ...
                    let jieguo = (1 - StringToFloat(str: Defaults[kSystemDeduct]!) / 100) * StringToFloat(str: OCTools().getStrWithFloatStr2(weakSelf?.GoodsInformationModel.final_price)) * StringToFloat(str: OCTools().getStrWithFloatStr2(weakSelf?.GoodsInformationModel.commission_rate)) / 100 * StringToFloat(str: Defaults[kCommissionRate]!) / 100
                    
                    let coupon_price : NSString =  OCTools().getStrWithIntStr(weakSelf?.GoodsInformationModel.coupon_price)! as NSString
                    
                    let mm :Double = coupon_price.doubleValue + Double(jieguo)
//                    let sMoney = String(format: "%.2f", mm)
                    
                    weakSelf?.fenxiangBun.setTitle("分享赚￥" + String.init(format:"%.2f",jieguo) + "  ", for: .normal)
                    weakSelf?.goumaiBun.setTitle("自购省￥" + String.init(format:"%.2f",mm) + "  ", for: .normal)
                }
...
//    请求猜你喜欢的数据
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView.tag == 1021 {
            ...
            navigaView.backgroundColor = kSetRGBAColor(r: 0, g: 0, b: 0, a: scrollView.contentOffset.y / navHeight / 2 * 3)
        }
    }
    // 收藏
...
//        }
    ...
    func tableView(_ tableView: UITableView, viewForFooterInSection section: Int) -> UIView? {
        ...
        if section == 0 {
            if newGuessModels.count <= 0 {
                let lab = UILabel.init(frame: CGRect.init(x: 0, y: 10, width: kSCREEN_WIDTH, height: 50))
                lab.text = "商品详情"
                lab.textColor = UIColor.white
                lab.backgroundColor = UIColor.black
                lab.textAlignment = .center
                lab.font = UIFont.font(16)
                foot.addSubview(lab)
            }
        } else if section == 1 {
            let lab = UILabel.init(frame: CGRect.init(x: 0, y: 10, width: kSCREEN_WIDTH, height: 50))
